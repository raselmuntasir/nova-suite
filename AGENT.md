...
* Key Libraries: Framer Motion, React Router, Lucide React

🌍 Deployment & Portability
* Currently hosted on Vercel (Frontend) and Supabase (Backend/DB) for free.
* Ensure code and database structures are vendor-agnostic where possible.
* Maintain clear database schemas (e.g., supabase_schema.sql) so that the database can be easily migrated to Hostinger or other custom PostgreSQL hostings in the future.
* Avoid heavy reliance on platform-specific lock-ins that would make switching hosting providers difficult.
